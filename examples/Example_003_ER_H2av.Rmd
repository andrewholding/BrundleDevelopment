---
title: "Normalisation Example using Spike-in Control as a Standard"
...
This is a normalisation using H2av binding within an external spike-in of drosophilia chromatin to provide a control for the analysis of ER binding in MCF7 cells after treatment with Fulvestrant. 

This example builds on the previous two examples and therefore they should be read first as many details of key functions have not been repeated here. 
...
To allow us to use the merged genome we first indexed it using bowtie2. For our system, we specified 32 threads; the number of threads will depend on the number of processors you have available. 
...
Once indexed, we could then align to the merged genome. This script assumes that the FastQ files are in a directory ./SLX-8047_dhms from where the scripts are run.

```{bash, eval=FALSE}
#Align, Sort and Build indexes
...
After aligning the data and calling peaks, we can then split the data back into human and drosophila. 
...
With the peak files and the separate alignments extracted, we can then write sample sheets. For the normalization we need one for the drosophila aligned reads from each sample and the respective H2av peak file, and one for human reads from each sample and their respsective ER peak files. This forms the bais of the following analysis.
...
As before, to keep file size down, data are provided in Rdata format rather than as raw BAM files.
