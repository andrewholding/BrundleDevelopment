...
To prepare the data for the differential binding analysis the data had to be aligned and CTCF and ER peaks called. As our CTCF control doesn't require any special aligment, data was aligned as for a standard ChIP-seq experiment. Peak calling was done with macs2 using the script shown below. 
...
As macs2 cannot differentiate between a peak arising from CTCF binding or ER binding we used two samples were squence to establish a CTCF binding consensus. One treated and one untreated. These peaks were then merged as follows.
...
These were then used to write two DiffBind samplesheets as described in the DiffBind vignette. One using the CTCF peak list generated from the above scripts, this is the control sample sheet. The second uses the ER peaks, this is the experimetnal sample sheet. These two sample sheets are then provided as the input in the analysis below. 
...
Brundle provies a series of functions we developed for this pipeline that are used to normalizing ChIP-seq data. The usage of these functions are explained in the example that follows.
...
Typically we would load the raw data using Diffbind as we are plan to return our normalized data as a Diffbind object. 

In this example, as the BAM files which are greater in size than the github filesize limit, we have included the data as the Diffbind object these function would output and instead load that directly. If the code doesn't find the R data object it will try to regenerate if from the raw data.  
...
Finally we return the correct values into the DiffBind object. This requires a modificed version Diffbind as included in the original manuscript. It is expected that this will be supported natively by furture versions of Diffind. 
